...
const getGradientStyle = (gradientId) => {
    ...
    return {
        ...
        boxShadow: `0 0 12px ${grad.from}55`
    };
};
...
const Results = ({ myWPM, finalPlayers, handleExit, handlePlayAgain }) => {
    ...
    const leaderboard = Object.entries(finalPlayers || {}).map(([id, data]) => {
        ...
        return {
            ...
            isMe: id === socket.id,
            name: data.name || null,
            avatarGradient: data.avatarGradient || 'purple-blue',
            wpm: finalWPM || 0,
                        ...
                        >
                            ...
                            <div className="col-span-6 flex items-center gap-3">
                                <div
                                    ...
                                    style={getGradientStyle(player.avatarGradient)}
                                >
                                    {initial}
                                </div>
                                <span className={`font-bold text-lg ${player.isMe ? 'text-blue-400' : 'text-gray-300'}`}>
                                    {player.name ? `${player.name}${player.isMe ? ' (You)' : ''}` : (player.isMe ? 'You' : 'Guest')}
                                </span>
                            </div>
